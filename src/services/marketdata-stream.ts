...
import {
  MarketDataStreamServiceServiceImplementation,
  MarketDataRequest,
  MarketDataResponse,
  CandleSubscription,
  SubscriptionStatus,
...
export class MarketDataStream implements MarketDataStreamServiceServiceImplementation {
  ...
  protected async emitCandle() {
    for (const { figi, interval } of this.subscriptions.candles) {
      const historicCandle = await this.broker.marketdata.getCurrentCandle(figi);
  ...
  protected async emitLastPrice() {
    for (const { figi } of this.subscriptions.lastPrice) {
      const { close: price, time } = await this.broker.marketdata.getCurrentCandle(figi);
  ...
  protected async emitOrderBook() {
    for (const { figi, depth } of this.subscriptions.orderBook) {
      const { high: limitUp, low: limitDown, time } = await this.broker.marketdata.getCurrentCandle(figi);
