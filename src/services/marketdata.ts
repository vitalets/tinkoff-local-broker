...
import {
  MarketDataServiceServiceImplementation,
  GetCandlesRequest,
  HistoricCandle,
...
import { CandlesReq, CandlesReqParams } from 'tinkoff-invest-api/dist/candles-loader/req.js';
import { Broker } from '../broker.js';

const debug = Debug('tinkoff-local-broker:marketdata');
const candlesLoaderDebug = Debug('tinkoff-local-broker:candles-loader');
...
export class MarketData implements MarketDataServiceServiceImplementation {
  private candles: Map</*figi*/string, HistoricCandle[]> = new Map();
  private candlesFromTo: Map</*figi*/string, DateRange> = new Map();
  ...
  async getCandle(figi: string, offset: number) {
    ...
    const candlesReq = new CandlesReq(
      this.broker.realApi,
      this.options,
      { figi, minCount, to, interval: this.interval },
      candlesLoaderDebug
    );
