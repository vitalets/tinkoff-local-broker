...
import {
  ...
  Operation,
  OperationsRequest,
  OperationsServiceServiceImplementation,
  PortfolioPosition,
  PortfolioRequest,
  PortfolioResponse,
  PositionsRequest,
...
export class Operations implements OperationsServiceServiceImplementation {
  ...
  private async updatePositionsCurrentPrice() {
    ...
    const { lastPrices } = await this.broker.marketdata.getLastPrices({ figi });
